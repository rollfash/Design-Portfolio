...
export function Signature({ progress, className }: SignatureProps) {
  ...
  // Calculate path length once on mount
  useEffect(() => {
    if (pathRef.current) {
  ...
  // Update dashoffset based on scroll progress
  useEffect(() => {
    ...
    const unsubscribe = progress.on('change', (latest) => {
      ...
      // Map progress 0.1-0.9 to full path draw
      const drawProgress = Math.max(0, Math.min(1, (latest - 0.1) / 0.8));
      const offset = shouldReduceMotion ? 0 : pathLength * (1 - drawProgress);
      
      pathRef.current.style.strokeDashoffset = `${offset}`;
      pathRef.current.style.opacity = drawProgress > 0 ? '1' : '0';
    });

    return () => unsubscribe();
  }, [progress, pathLength, shouldReduceMotion]);
  ...
  return (
    <div className={cn("pointer-events-none select-none flex items-center justify-center transform scale-50", className)}>
      <svg
