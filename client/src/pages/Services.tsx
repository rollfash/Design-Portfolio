    ...
    <Layout>
       <div className="container px-6 py-20">
               ...
               <ul className="space-y-3 mb-8">
                 {service.features.map((feature) => (
                   <li key={feature} className="flex items-center text-sm">
                     <Check className="h-4 w-4 ml-3 text-primary/50" /> {feature}
                   </li>
                 ))}
               </ul>
               <Link href="/contact">
                 <Button variant="outline" className="w-full">לפרטים נוספים</Button>
               </Link>
             </div>
           ))}
         </div>
         ...
         <div className="bg-primary text-primary-foreground p-12 md:p-24 text-center">
            ...
            <p className="text-primary-foreground/70 mb-8 max-w-xl mx-auto">בואו נדבר על הפרויקט שלכם ונראה איך אפשר להפוך את החזון למציאות.</p>
            <Link href="/contact">
              <Button size="lg" variant="secondary" className="px-8">תיאום פגישת ייעוץ</Button>
            </Link>
         </div>
       </div>
    </Layout>
